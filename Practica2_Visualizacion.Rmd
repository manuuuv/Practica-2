...
#Vamos a cargar el fichero utilizando la función load:
load("data/weatherAUS.RData")
#A continuación,creamos el objeto ggplot llamado como grafico1:
grafico1 = ggplot(data=ds,aes(x=MinTemp,y=MaxTemp))
#Definimos el objeto geométrico:
grafico1 + geom_point()
#Añadimos el color a una variable llamada grafico2:
grafico2 = ggplot(data=ds,aes(x=MinTemp,y=MaxTemp,col=RainTomorrow))
grafico2 + geom_point()
#Ponemos una nueva capa con el título del gráfico y lo guardamos en la variable grafico3:
grafico3 = ggplot(data=ds,aes(x=MinTemp,y=MaxTemp,col=RainTomorrow)) + ggtitle("Representación de las temperaturas máximas y mínimas en función de si llueve mañana o no") + geom_point()
#Guardamos el gráfico resultante en un fichero pdf:
...
ggplot(data=ds,aes(x=Temp9am,y=Pressure9am)) + geom_point()
#Modificamos el gráfico para modificar la forma del punto en función de una variable categórica:
ggplot(data=ds,aes(x=Temp9am,y=Pressure9am,shape=RainToday)) + geom_point()
ggplot(data=ds,aes(x=Temp9am,y=Pressure9am,col=RainToday)) + geom_point()
#Se observa mejor la información con el color puesto que se diferencia los puntos con mayor claridad
#Añadimos el título al gráfico:
ggplot(data=ds,aes(x=Temp9am,y=Pressure9am,col=RainToday)) + geom_point() + ggtitle("Representación de la temperatura frente a la presión a las 9 de la mañana")
#Guardamos el gráfico en un fichero pdf:
...
str(iris.tidy)
#Vemos que el conjunto de datos contiene 4 variables y 600 observaciones.
#Creamos el grafico iris.grafico1:
iris.grafico1 = ggplot(data=iris.tidy,aes(x=Species,y=Value))
iris.grafico1 + geom_point()
#Hacemos el grafico iris.grafico2 modificando el primero:
iris.grafico2 = ggplot(data=iris.tidy,aes(x=Species,y=Value,col=Part))
#Representamos el grafico iris.grafico2:
iris.grafico2 + geom_point()
#Represento el gráfico con facet_grid en forma de fila:
ggplot(data=iris.tidy,aes(x=Species,y=Value,col=Part)) + geom_point() + facet_grid(Measure~.)
#Represento el gráfico con facet_grid en forma de columna:
ggplot(data=iris.tidy,aes(x=Species,y=Value,col=Part)) + geom_point() + facet_grid(.~Measure)
#Se compara mejor las variables en forma de columna.
#Cambiamos por geom_jitter:
ggplot(data=iris.tidy,aes(x=Species,y=Value,col=Part)) + geom_jitter() + facet_grid(.~Measure)
...
str(DatosEjer4)
#200 observaciones y 3 variables.
#Representamos el gráfico de puntos:
ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,col=Fabricante)) + geom_point()
#No es válido puesto que con los puntos no se ve la evolución
ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,col=Fabricante)) + geom_point() + geom_line()
#Tendencia de cada fabricante con un tipo de línea distinto.
ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,col=Fabricante)) + geom_line(aes(linetype=Fabricante)) + geom_point()
#Le añadimos el título al gráfico:
ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,col=Fabricante)) + geom_line(aes(linetype=Fabricante)) + geom_point() + ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
#Guardamos el gráfico en un fichero pdf:
...
#Cargamos el fichero y miramos sus variables:
load("data/Experimento.Rdata")
str(Experimento)
#Representamos el gráfico de puntos:
ggplot(data=Experimento,aes(x=Dosis,y=Altura,linetype=Complemento,shape=Complemento,col=Complemento)) + geom_line() + geom_point()
#El gráfico con barras de error a cada punto:
ggplot(data=Experimento,aes(x=Dosis,y=Altura,linetype=Complemento,shape=Complemento,col=Complemento)) + geom_line() + geom_point() + geom_errorbar(aes(ymin=Altura-Error_Altura,ymax=Altura+Error_Altura))
#Gráfico idéntico al anterior con paleta de color rojos:
ggplot(data=Experimento,aes(x=Dosis,y=Altura,col=Complemento)) + geom_line(aes(linetype=Complemento)) + geom_point(aes(shape=Complemento)) + geom_errorbar(aes(ymin=Altura-Error_Altura,ymax=Altura+Error_Altura)) + theme_classic() + scale_color_brewer(palette="Reds")
#Guardamos el gráfico:
...
str(ds)
#Creamos el boxplot:
ggplot(data=ds,aes(x=Year,y=MinTemp,fill=Year))+ geom_boxplot()
#Creamos el gráfico de violines:
ggplot(data=ds,aes(x=Year,y=MaxTemp,fill=Year))+ geom_violin() + labs(title="Distribución de temperaturas máximas por año",x="Año", y = "Temperatura (ºC)") 
#Eliminamos la leyenda:
ggplot(data = ds, aes(x=Year, y=MaxTemp, fill=Year))+ geom_violin() + labs(title="Distribución de temperaturas máximas por año",x="Año", y = "Temperatura (ºC)") +theme(legend.position='none', axis.text.x=element_text(angle=45, hjust=1))
...
library(readr)
df_datacamp = read.table("http://assets.datacamp.com/blog_assets/chol.txt",header=TRUE)
str(df_datacamp)
#Histograma de la variable HEIGHT:
ggplot(data=df_datacamp, aes(x=HEIGHT)) + geom_histogram()
#Histograma con etiqueta y título:
ggplot(data=df_datacamp, aes(x=HEIGHT)) + geom_histogram() + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia")
#Ajustando el rango de representación:
ggplot(data=df_datacamp, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155, 195, by=2) ) + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia") 
#Barras de color azul y contorno rojo:
ggplot(data=df_datacamp, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155, 195, by=2),col="blue",fill="red",alpha=0.5 ) + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia") 
#Curva de distribución:
ggplot(data=df_datacamp, aes(x=HEIGHT)) + geom_histogram(aes(y =..density..),breaks=seq(155, 195, by=2), col= "blue",fill="red", alpha=0.5 ) + geom_density(col="green") + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia")
#Grafico para cada valor de SMOKE:
ggplot(data=df_datacamp, aes(x=HEIGHT)) + geom_histogram(aes(y =..density..),breaks=seq(155, 195, by=2), col= "blue",fill="red", alpha=0.5 ) + geom_density(col="green") + labs(title="Histograma de la variable Altura",x="Altura (cm)", y = "Frecuencia") +
facet_grid(.~SMOKE)
#Guardamos el gráfico:
